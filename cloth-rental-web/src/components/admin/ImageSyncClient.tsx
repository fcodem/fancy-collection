...
import { syncInventoryPhoto } from "@/app/admin/image-sync/actions";
...
function stripExtension(filename: string): string {
  ...
  return (i > 0 ? basename.slice(0, i) : basename).trim();
}
...
const STATUS_CONFIG: Record<FileStatus, { color: string; icon: string; label: string }> = {
  ...
  processing: { color: "#1565c0", icon: "fa-spinner fa-spin", label: "Processing" },
  success: { color: "var(--success, #2e7d32)", icon: "fa-circle-check", label: "Success" },
  failed: { color: "var(--danger, #c62828)", icon: "fa-circle-xmark", label: "Failed Match" },
};
...
export default function ImageSyncClient() {
  ...
  async function startUpload() {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      updateFile(i, { status: "processing", message: "" });
      ...
      try {
        ...
        const result = await syncInventoryPhoto(form);

        if (result.ok && result.matched) {
          updateFile(i, {
            status: "success",
            message: `${result.itemName} (${result.sku})`,
          ...
          updateFile(i, {
            status: "failed",
            message: !result.ok ? result.error : "No match found",
          });
        }
      } catch {
  ...
  const failedMatches = files.filter((f) => f.status === "failed");
  ...
  return (
    <div>
      <div className="card" style={{ marginBottom: 20 }}>
        ...
        <div className="card-body">
          <p style={{ margin: "0 0 16px", fontSize: 13, color: "var(--text-muted)" }}>
            Drop a <strong>.zip</strong> of inventory photos. Each filename should match an inventory dress name
            (e.g. <code>GOLDEN BRIDAL.jpg</code>). Images are matched and uploaded one at a time via a secure server
            action.
          </p>
          ...
          {files.length === 0 && !extracting && (
            ...
            >
              <i
                ...
                style={{ fontSize: 48, color: "var(--primary, #7b1f45)", marginBottom: 12, display: "block" }}
              />
              <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 6 }}>Drop your ZIP file here</div>
        ...
        <div className="card" style={{ marginBottom: 20 }}>
          ...
          >
            <h3 className="card-title" style={{ margin: 0 }}>
              ...
              {zipName} · {counts.total} images
            </h3>
            <div style={{ display: "flex", gap: 8 }}>
              {!processing && counts.pending > 0 && (
                <button type="button" className="btn btn-primary btn-sm" onClick={startUpload}>
                  <i className="fa-solid fa-play" style={{ marginRight: 6 }} />
                  Start Upload
                </button>
              )}
              {processing && (
                <button type="button" className="btn btn-outline btn-sm" onClick={stopUpload} style={{ color: "var(--danger)" }}>
            ...
            <div style={{ maxHeight: 480, overflowY: "auto" }}>
              {files.map((f, i) => {
                ...
                return (
                  ...
                  >
                    <i className={`fa-solid ${cfg.icon}`} style={{ color: cfg.color, width: 16, textAlign: "center" }} />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {f.message && (
                        <div
                          style={{
                            fontSize: 11,
                            color: f.status === "success" ? "var(--success)" : "var(--text-muted)",
                            marginTop: 2,
                          }}
                        >
                          {f.message}
                    ...
                    <span style={{ fontSize: 11, fontWeight: 700, color: cfg.color, textTransform: "uppercase", flexShrink: 0 }}>
                      {cfg.label}
                    </span>
                  </div>
                );
      ...
      {!processing && failedMatches.length > 0 && counts.pending === 0 && (
        <div className="card" style={{ marginBottom: 20, borderLeft: "4px solid var(--danger)" }}>
          ...
          <div className="card-body" style={{ padding: 0 }}>
            {failedMatches.map((f, i) => (
              <div key={i} style={{ padding: "8px 16px", borderBottom: "1px solid var(--border)", fontSize: 13 }}>
                <strong>{f.dressName}</strong>
                {f.message && (
      ...
      {!processing && counts.success > 0 && counts.pending === 0 && (
        <div className="card">
          <div className="card-body" style={{ textAlign: "center", padding: 24 }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 700 }}>Sync Complete</div>
            <div style={{ fontSize: 13, color: "var(--text-muted)", marginTop: 4 }}>
              {counts.success} image{counts.success !== 1 ? "s" : ""} matched and uploaded
              {counts.failed > 0 && <span> · {counts.failed} could not be matched</span>}
            </div>
          </div>
        </div>
      )}
