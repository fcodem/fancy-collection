...
export async function GET(req: NextRequest) {
  ...
  const [exact, before, after, rest] = await Promise.all([
    prisma.booking.findMany({ where: { ...base, ...exactWhere }, include: bookingListInclude, orderBy: { returnTime: "asc" }, take: 100 }),
    prisma.booking.findMany({ where: { ...base, ...beforeWhere }, include: bookingListInclude, orderBy: { returnTime: "asc" }, take: 100 }),
    prisma.booking.findMany({ where: { ...base, ...afterWhere }, include: bookingListInclude, orderBy: { returnTime: "asc" }, take: 100 }),
