...
export async function retrySafeFailedAiJobs(limit = 50): Promise<number> {
  ...
  const deterministic =
    /invalid size|SIGABRT|heap|out of memory|ENOMEM|ENOSPC|no space left|Input image exceeds|limitInputPixels|deterministic/i;
  let n = 0;
  for (const job of failed) {
    const msg = `${job.lastError || ""} ${job.errorMessage || ""}`;
    if (deterministic.test(msg)) continue;
