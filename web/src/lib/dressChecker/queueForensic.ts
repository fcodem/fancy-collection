...
export async function buildQueueForensicReport(): Promise<QueueForensicReport> {
  ...
  const [worker, queue, pgvector, jobsTable, heartbeatTable] = await Promise.all([
  ...
  if (worker.status === "OFFLINE") warnings.push("Worker heartbeat offline (>10m)");
  else if (worker.status === "DEGRADED") warnings.push("Worker heartbeat degraded (3–10m)");

  const [stuckProcessing, oldestPending, avgProc, profileCounts] = await Promise.all([
